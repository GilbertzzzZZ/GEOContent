...
| 商业查询 | "A vs B 哪个好" | 对比表格 + 社会证明 + 专家观点 | AI Overviews 商业覆盖升至 19% |
...
- [ ] 确认 robots.txt 允许 GPTBot/CCBot/Claude-Web

**P1（重要）**：
- [ ] FAQ 区域覆盖 3-5 个常见长尾问题
- [ ] 品牌核心定位词在文中自然出现 3+ 次
- [ ] 社会证明元素（用户数、评价、排名）
- [ ] 图片配备语义化 Alt Text 和 Caption
- [ ] 服务端渲染（SSR），非客户端渲染
- [ ] 内容长度 500-2,000 字（LinkedIn 被引最多区间）
...
| 渠道类型 | 代表平台 | 优先级 | 影响维度 | 难度 |
|---------|---------|-------|---------|-----|
| 技术基础 | robots.txt, SSR | P0 | 全平台可见性前提 | 低 |
| 自有官网/博客 | 品牌网站 | P0 | 内容质量+结构化 | 中 |
| 第三方权威媒体 | 路透社, FT, AP | P1 | 训练数据+权威信号 | 高 |
| Wikipedia | 维基百科 | P1 | ChatGPT 引用 48% | 高 |
| Reddit/Quora/论坛 | Reddit, Quora | P1 | Perplexity 47%, AIO 21% | 中 |
| LinkedIn | LinkedIn | P1 | AI 引用第二多域名 | 中 |
| 行业目录/聚合 | G2, Capterra, PH | P2 | 发现查询+品牌知名度 | 中 |
| 学术/研究内容 | arXiv, 白皮书 | P2 | 深度权威+训练数据 | 高 |
| 社交媒体 | Twitter/X, YouTube | P3 | YouTube 占 AIO 19% | 中 |
...
**建议**：维护完整的聚合平台 profiles，但不要过度投入。优先级低于 Wikipedia、Reddit、权威媒体。
...
| 平台 | 首要引用源 | 偏好特征 | 索引基础 | 用户规模 |
|------|----------|---------|---------|---------|
| ChatGPT Search | Wikipedia (47.9%) | 知识权威导向 | Bing 索引 | 3亿+ |
| Perplexity | Reddit (46.7%) | 社区讨论导向 | 独立爬虫 | 快速增长 |
| Google AI Overviews | Reddit (21%), YouTube (18.8%), Quora (14.3%), LinkedIn (13%) | 多元化分布 | Google 索引 | 全球最大 |
| Claude | — | 训练数据导向 | 有限实时检索 | 增长中 |
